"""
Jobs API — The core of FCIP.

POST /jobs         → Create job + run profit prediction immediately
GET  /jobs         → List all jobs for fleet
GET  /jobs/{id}    → Get single job
PATCH /jobs/{id}/status  → Accept / Reject / Complete
PATCH /jobs/{id}/actual  → Record actual revenue + cost after completion
"""
...
async def update_job_actual(
    ...
    """Record actual revenue and cost after job completion. Used to improve prediction accuracy."""
    repo = JobRepository(db)
    job = await repo.get(job_id, current_user.fleet_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
